...
- 文档声明的可执行输入合同（flag、SHA 长度、enum、路径或格式）必须与 parser 和边界测试逐值一致；不能用“完整”描述一类输入、却让实现和测试接受缩写或更宽区间。合同变化时 docs/parser/tests 同一提交更新。
- 编辑同时承载多个独立约束的复合规则时，必须对照 base 检查被删除的政策关键词；只删除需求明确废止的子句，其他隐私、发布或平台边界必须原样保留或等价迁移。
- 历史 task / research 纳入 Git 后仍是历史证据：保留原结论，但若其中的现在时环境描述已失效，应增加“当时基线”等时间限定，避免旧状态被误读为当前规则。
