...
- 本候选性质：仅增加默认关闭、QA 显式启用的匿名聚合诊断（`--runtime-evidence=<sha>`）及其测试/privacy guard/文档；不修改障碍分类、alpha 阈值、candidate identity、control 几何、capture cadence、scheduler 架构、插值语义或权限请求。pre-Review admission 修复批次（identity 计数时机、stale 结果过滤、flush 写盘抑制）同样只改诊断 instrumentation 语义，不改产品分类/布局/调度行为。
...
| AC2c-4 聚合输入在生产 owner 处采集 | behavior（生产组合，plumbing-only） | fake capturer `.stats(expanded)`→`.targetMissing`；同 WID bounds 亚像素抖动（due / not-due / in-flight 三态）；control-kind fixture（60×24） | 基线 = v5 表 T-bv39/T-bv42 于 `24b9732` 通过（plumbing，正常路径无行为变化）；修复批次新增 T-re9（not-due jitter 不漏计）/T-re10（in-flight replacement 计数 + stale 结果拒绝）为 instrumentation coverage 补强，非红测、不改产品行为 | `BubbleVisibilityProbe.probe`（identity-change 在 capture gate 前同步计数；outcome/visibility 仅在 completion 通过 generation+capturedCandidates 接受并写 cache 后计入）→ `onVisibilityChange` → `FollowTickScheduler.visibilityChangeCallback` → 完整 tick → `FollowLayoutPass.placeDock`（kind/visible 计数）→ 真实 `DockPanel.placeBelow`（实际 frame 相对同 tick 无障碍 base 的 dy bucket） | T-re5a–d：capture/visibility/identity/wake 计数、tick/kind/visible 计数、dy upTo64→base 迁移、真实 frame 避让→复位；T-re6a/b：control 存在即占位 → bubble=1/control=1/visible=2；T-re9a/b：not-due jitter → identity+1 且 capture 调用数不变；T-re10a–c：in-flight jitter → identity+1、single-flight 不加捕获、stale 完成 0 计数、接受后的新捕获 1 计数 | 同上 | 真实 image3 的 runtime kind/outcome/identity 分布未采样（本表所有 fake 注入均为 plumbing-only） |
| AC2c-5 flush 写盘抑制（不干扰 moving 高频 tick） | behavior（可观察 flush 结果 + 实际文件内容） | 首个 layout 证据 → 100 个完全相同状态的 `recordLayoutTick`+`flush` → dy bucket 同值/新值 → layout 状态变化 → accepted capture/identity/wake | 基线 N/A：新增机制；修复前的"每 tick 写盘"是诊断自身开销缺陷（instrumentation 修复），非产品行为回归 | collector dirty 状态机（`State.dirty`）在 record 系列内判定有意义变化；`flush()` 先取 dirty 再决定是否进入 `PrivateStorage.atomicWrite`；无 Timer/Task/capture 新增（T-re8/pytest guard 复核） | T-re11a 首 evidence 写盘一次；T-re11b 100 个无变化 tick → 0 次写盘且文件 tickCount 停更；T-re11c/d dy bucket 新值写盘、同值不写；T-re11e layout 状态变化写盘；T-re11f accepted capture/identity/wake 写盘 | 同上 | 无 |
| AC2b（v6 状态重申） | plumbing-only | 既有 T-bv42 fake `.targetMissing` 注入 | 基线 = T-bv42 于 `24b9732` 基线通过（v5 表，plumbing）；触发等价性至今未证 | 既有生产组合证据不变（v5 表） | 管道能力已证；真实 full-hide 触发等价性未证 | `make test-ui` 273/273（含 T-bv42） | **待真机采样**：用本诊断候选执行真实图1→图2→图3，读 runtime-evidence.json 聚合后才可判定 H1/H2/H3/H4b 分支；在此之前不得宣称 image3 已修复 |
...
- flush 采用 dirty 抑制：仅 accepted capture / identity / wake / layout 状态变化 / dy bucket 变化触发写盘；无变化的高频 display tick 零写 IO；写盘失败恢复 dirty 以便下一 tick 重试。
