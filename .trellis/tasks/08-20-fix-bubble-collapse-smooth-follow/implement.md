...
- The controlling instructions for the current campaign are under **Eighth break-loop replan after v8 second Review**. Earlier Preconditions text, ordered-work, break-loop sections and Rollout descriptions are historical where incompatible; none overrides the eighth replan.
- Current product scope is the v9 private runtime-evidence implementation boundary only: a file-private sink-owning concrete type, an address-free recorder protocol existential, fixed-sink production/test facade factories, token-aware compiler-flag wiring, compiler mutations, AC Evidence Topology and directly affected docs. W7 and declaration/constructor inventories are deleted. No normal obstacle classification, telemetry schema/flush behavior, layout, scheduler, capture, interpolation or permission behavior may change.
...
## Eighth break-loop replan after v8 second Review

The v8 second formal Review ended with P0=0/P1=1/P2=1, so v8 is closed without QA and no third patch is allowed. Start a fresh v9 campaign from the committed v9 planning baseline:

1. Preflight one fresh `zhipu/glm-5.3 + max` implementation owner in a unique clean worktree at the exact planning SHA. Do not reuse v8 implementation or Review agents.
2. Reproduce the two v8 Review holes before implementation: an unflagged `convenience init` and `static subscript` in the concrete collector must compile and leave W0-W7 green; a second `-D PETDOCK_TESTING` in another swiftc recipe must leave the old W5 green. Restore the base completely after recording the red baselines.
3. Introduce an internal `RuntimeEvidenceRecording` protocol containing only the existing record/snapshot/flush capabilities. Make `RuntimeEvidenceCollector` itself `private final` and keep all sink state and construction inside `RuntimeEvidence.swift`. Production consumers in `main.swift`, `BubbleVisibility.swift`, `FollowTickPlan.swift` (`FollowLayoutPass`) and `DockPanel.swift` depend only on `any RuntimeEvidenceRecording`.
4. Replace the concrete static entries with exact same-file facade names: `makeRuntimeEvidenceRecorder(candidateSHA:flushNow:) -> any RuntimeEvidenceRecording` for production and `makeRuntimeEvidenceRecorderForTesting(candidateSHA:outputURL:flushNow:) -> any RuntimeEvidenceRecording` only inside `#if PETDOCK_TESTING`. The production facade hard-codes the private Diagnostics sink. If fixtures need the filename, expose only a filename constant without a URL/path capability. Rewrite both the concrete `forTesting` calls and `RuntimeEvidenceCollector.outputFileName` references in `tests/main.swift`; do not expose the concrete type or output address through the protocol.
5. Delete old W0, W3, W6 and W7: private-init shape, URL token allowlist, static-production shape and declaration inventory are all superseded and must not be re-pinned with source regex. Retain/update W1/W2 for the exact test-facade token and flag region, retain W4 Package absence, and replace W5 with token-aware inspection of every Makefile swiftc command. It must recognize both `-DNAME` and `-D NAME`, reject malformed dangling `-D`, and assert `PETDOCK_TESTING` occurs exactly once in the test-ui recipe. Add real compiler-probe tests for external concrete-type naming/construction, production-facade address arguments and release test-facade absence. Keep aggregate-only guards and ASCII SHA tests.
6. Mutation matrix must include: external naming/construction of the private concrete type; v8 `convenience init` and `static subscript` added to the private type while external access still fails; production factory passed an address; release reference to the test factory; Package define; duplicate connected and separated Makefile defines. Every mutation must FAIL for the intended layer, then restore to PASS.
7. Do not alter telemetry fields, flush cadence, capture/generation semantics, obstacle classification, alpha thresholds, scheduler, interpolation, permissions or UI behavior. Existing image3 tests remain plumbing-only. Update docs/spec/task evidence without real paths or commit fingerprints.
8. The implementation owner runs the writable `trellis-check` checklist, complete gates and repeated test-ui, then freezes a full SHA. Main admission audits each compiler/wiring mutation before a fresh zhipu formal Review. One consolidated repair batch and one final Review remain the maximum; a second Review finding triggers another break-loop.
9. Review clear → fresh zhipu automated QA builds and archives the exact candidate → fresh `kimi/k3 + max` visual QA closes the old App, launches the archived candidate and performs image 1→2→3. No visual dispatch or image3 fix claim before Review and automated QA clear.
