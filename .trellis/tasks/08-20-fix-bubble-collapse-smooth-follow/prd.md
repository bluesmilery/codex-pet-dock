...
- 精确候选 `91a8fe6ba915f84e35f232943fd1c1c3a558063d` 的自动 Review/QA 已通过，但用户真机验证区分出两条不同状态链：消息卡片仍存在时 expanded→collapsed 能正确复位；控制按钮和全部消息窗口一起隐藏后，底座仍跟随宠物移动却保持旧避让间距。旧候选因此未 accepted、未合入 `dev`。
...
- 完全隐藏会话 UI 时，即使 CGWindowList 暂时仍返回旧几何候选，只要一次成功取得的 ScreenCaptureKit 窗口清单明确表明目标 WID 已不存在，就必须把该候选从障碍集中失效并唤醒完整布局；不得继续保持旧避让间距。
- 捕获结果必须区分“目标窗口已不在一次成功取得的窗口清单中”和“权限、清单获取、截图或像素统计失败”。只有前者可作为 hidden 事实；其余失败继续保守 `.visible`。保留 generation/strict single-flight、当前候选身份校验和 TCC preflight gate 语义。
...
- 在 moving 状态对底座当前位置到最新目标 frame 使用短时线性插值，按单调时间计算进度并在显示节拍上渲染；新目标到达时只追最新目标，不排队重放历史位置，不允许过冲。插值时长必须有明确上限，停止移动后精确落到最终目标。
...
- [ ] AC2b：集成 harness 覆盖“expanded 可见候选 → CG 候选仍短暂存在但一次成功 SCK 列表已无目标 → hidden 通知 → 完整布局无障碍”；底座回到基础 frame。清单/截图失败的相邻用例仍保持保守避让。
...
- [ ] AC5b：线性插值在 60 Hz、120 Hz 和不规则节拍下使用同一时长语义，单调趋近最新目标、无过冲、无历史位置队列，并在配置的最大拖尾窗口内精确到达最终 frame；安全复位路径可立即 snap。
...
- [ ] AC8：本任务后续所有实现、正式 Review、修复复核和 QA 子 Agent 使用全新 `xai/grok-4.6 + high`；任何一次模型/推理不可用都停止派发，不静默降级。Reviewer 对冻结完整 SHA 报告 P0/P1/P2=0 后，QA 才可对同一 SHA 重跑门禁。
...
- 线性插值允许的最大视觉拖尾尚需用户确认。推荐 `32ms`：普通 60 Hz 约两帧，能柔化离散位置跳变，同时明显小于现有 0.1 秒稳定窗口；选择更长会更柔和但跟手性更差，选择 `0ms` 则保留当前直接 setFrame 行为。
