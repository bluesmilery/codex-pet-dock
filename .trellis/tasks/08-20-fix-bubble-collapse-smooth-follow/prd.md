...
- [ ] AC2c：诊断模式默认关闭，关闭时不创建诊断文件、不增加 capture 或 timer；显式启用只接受恰好 40 个 ASCII 小写十六进制字符（`0-9` / `a-f`）的完整候选 SHA，并只产生隐私白名单内的时间窗聚合。dy bucket 必须由 `setFrame` 后回读的真实 `DockPanel.frame` 计算，不得消费请求 frame。持有 `outputURL` 的具体 collector 类型必须整体文件私有；生产文件只能依赖不含地址能力的 recorder 协议，并经无 URL 参数、固定 `PrivateStorage.diagnosticsURL` sink 的生产工厂取得 existential。测试自定义 sink 只在 test-ui 专用编译 flag 下存在、release 不可见。编译 mutation 必须证明外部文件无法命名/构造具体类型，且任意成员声明形态不会扩大生产 API；flag/wiring guard 必须按 shell token 同时识别 `-DNAME` 与 `-D NAME`。上述 mutation 均须 FAIL → 撤销后 PASS；不得枚举 Swift declaration/constructor 正则或用自制 parser 代替语言约束。输出权限保持 0700/0600、no-follow，禁止字段与 symlink 回归继续通过；诊断候选本身不改变正常用户路径的障碍分类或布局行为。
