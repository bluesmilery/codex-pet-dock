...
L2: dispatch one `trellis-implement` owner. That same implementer uses the `trellis-check` skill/checklist for writable self-check and delivers an AC Evidence Topology Table before freezing a full candidate SHA; the main Agent must reject broken production paths, unused fakes, or tests that stop before the final state owner before formal Review. Do not dispatch a writable `trellis-check` Agent as formal Review. Then use a fresh read-only Agent with the resolved model configuration for exhaustive formal Review of that SHA. If clean, run QA; if findings exist, return one consolidated repair batch to the same implementer and perform one final Review on the new SHA. Any substantive finding still present in the second Review triggers `trellis-break-loop` and replanning instead of a third Review round.
...
First classify the current request. An L0 status/read-only request inside an active task is answered directly without advancing task status or entering the execution loop. Otherwise read `Delivery Path` from `prd.md`; missing or unclear path fails closed to L2.
L1: `trellis-before-dev` if applicable -> main-session edit -> targeted static checks -> one fresh read-only consistency check using the resolved model configuration -> Phase 3.3/3.4. Batch any findings once; escalate to L2 if runtime or executable impact appears.
L2: follow the project's sub-agent/worktree rules even when Codex inline mode is configured. The original implementer uses the `trellis-check` skill/checklist for writable self-check and delivers an AC Evidence Topology Table; the main Agent audits each production path and final state owner before dispatching formal Review. Formal Review is a separate fresh read-only Agent using the resolved model configuration on a frozen SHA, followed by QA only when P0/P1/P2 are clear. One consolidated repair batch and one final Review are allowed; any substantive finding still present in the second Review triggers `trellis-break-loop` and replanning instead of a third Review round.
After the L1 consistency gate or L2 Review+QA gate is accepted: complete the Phase 3.3 `trellis-update-spec` judgment, present the Phase 3.4 one-shot commit/integration plan, then run `/trellis:finish-work` only after the commit step is resolved.
...
- Persist an **AC Evidence Topology Table** at `{TASK_DIR}/research/ac-evidence-topology.md` and include it in the candidate commit. Use one row per acceptance criterion and per user-reported symptom. Each row records: symptom/AC, evidence type, test trigger or perturbation, approved product-baseline full SHA, baseline test state (clean checkout plus exact test-only patch/commit and changed paths when needed), baseline result, actual production consumer, callback/scheduler path when applicable, final state owner and observable assertion, command/result, and explicit manual-only gap.
- For every user-reported symptom, include at least one regression that traverses the real production composition through the final state owner. Helper, pure-function, or local sink tests may supplement this row but cannot satisfy it alone.
- Prove every injected fake, clock, event, failure, and callback is consumed by the system under test. Mutating a test-local value that no production dependency reads is invalid evidence. For absence contracts, combine behavior evidence with an executable source/API guard instead of injecting an unused forbidden dependency.
- Run the symptom regression against the approved unmodified baseline before changing product code and record the result. A failing baseline supports a behavior fix; a passing baseline identifies a coverage-only evidence gap and does not justify speculative product changes. Never fabricate a red test by bypassing production composition.
- Map all remaining acceptance criteria to evidence appropriate to their type: production-owner assertions for behavior, direct commands for build/docs, executable guards for static/absence contracts, or explicit real-device/manual QA.
...
The main Agent owns a separate **pre-Review admission check**. Before dispatching formal Review, it must read `ac-evidence-topology.md` from the exact candidate tree and inspect every AC row. For each behavior AC and user symptom, inspect the regression itself and trace every claimed edge from the test stimulus to the real production consumer and final state owner; for build/docs/static/absence ACs, verify the matching direct command or executable source/API guard. Independently resolve the recorded full baseline SHA, verify the stated clean or test-only baseline state and fail/pass result, and pass the evidence-file path plus frozen candidate SHA to the Reviewer. Test names, pass counts, helper return values, and implementer confidence are not sufficient. Reject the candidate back to the same implementation owner when a row or provenance field is missing, a fake is not consumed, an absence guard is missing, the test manually invokes a downstream helper, or a behavior assertion stops before the user-visible/persisted owner. This rejection is implementation self-check completion, not a formal Review round.
...
1. After targeted static checks pass, request one fresh Agent using the resolved model configuration to review the complete diff read-only against `prd.md`, `design.md` if present, and `implement.md` if present.
...
1. Dispatch a fresh formal Reviewer using the resolved model configuration in its own branch/worktree against the exact frozen candidate SHA. The Reviewer is strictly read-only and is not the implementation owner or the writable `trellis-check` Agent.
2. Review the complete assigned scope and every acceptance criterion. Read `ac-evidence-topology.md` from the exact candidate SHA; independently resolve the recorded baseline full SHA and verify its clean or test-only state and fail/pass result. For every behavior AC and user symptom, inspect the regression, confirm each perturbation is consumed by the real production object, trace callbacks/schedulers to the final state owner, and reject helper-only or local-sink substitutions. For build/docs/static/absence ACs, verify the matching direct command or executable source/API guard. Except when the candidate cannot be built/read or required evidence is missing so review cannot continue, do not stop after the first finding. Report all validated P0/P1/P2 findings together, plus covered scope, uncovered scope, commands, failures, and unverified items.
...
4. If the first Review has findings, the main Agent deduplicates them into one repair batch. Return the whole batch to the same implementation owner, who groups by root cause, searches analogous locations, updates the AC Evidence Topology Table and regression evidence, reruns `trellis-check` self-check and the Review Readiness Gate, then freezes a new full SHA. All conclusions for the old SHA are invalid.
