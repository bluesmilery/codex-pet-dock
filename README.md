...
```sh
make test-ui      # pet detection + geometry + follow state machine + bubble visibility + obstacle avoidance (bubble + control button) + clamp + logger rotation + FollowTickPlanner (159 tests)
...
make test         # full suite (381 tests in total)
```
...
1. `make test` (full suite, 381 tests) passes;
...
- No remote URL or CI is fabricated in this document; all changes are validated by local `swift build -c release` and `make test` (381 tests).
