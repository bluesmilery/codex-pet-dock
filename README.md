...
- **Adaptive following**: follows the dock window's native display cadence on macOS 14+; macOS 13 uses a repeating timer selected from the current screen's refresh capability and capped at 120 Hz. Dense callbacks are coalesced to the latest pending tick, and stationary detection keeps the same elapsed-time meaning across 60/120 Hz and irregular cadence before stepping down to a 0.1-second idle check. It hides with the pet when the pet is hidden or Codex quits, and recaptures on reappearance.
