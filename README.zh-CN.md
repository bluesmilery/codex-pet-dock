...
make app          # 组装 build/PetDock.app；本机存在 'PetDock Local Development' 证书时自动稳定签名，否则 ad-hoc 签名（Identifier=io.github.bluesmilery.codexpetdock）
...
**分发**：当前发布包为本机签名（无 team ID）、未 notarized 的 arm64 预编译包。构建机存在 'PetDock Local Development' 证书时 `make app` 自动稳定签名，否则 ad-hoc 签名；实际签名方式、校验值与下载渠道以发布说明为准。

**预览安装**（非一键可信安装）：

1. 下载 `CodexPetDock-0.2.0-macOS-arm64.zip` 并解压。
2. 将 `PetDock.app` 移至 `/Applications`（或任意固定位置）。
3. 因应用为本机签名（稳定自签证书或 ad-hoc，均无 Developer ID、未公证），macOS Gatekeeper 可能拦截首次启动。前往**系统设置 › 隐私与安全性**，点击**仍要打开**（或在 Gatekeeper 对话框中选"打开"）。详见 [Apple 官方指南](https://support.apple.com/guide/mac-help/mh40616/mac)。
...
- **ad-hoc 回落时 TCC 不稳定**：本机没有 `PetDock Local Development` 证书时，`make app` 回落 ad-hoc 签名；每次重新签名都会改变代码目录哈希，导致屏幕录制授权失效，需重新授予。
