...
```sh
make test-ui      # 宠物识别 + 坐标转换 + 跟随状态机 + 气泡可见性 + 障碍避让 + 边缘 clamp + 日志轮转 + FollowTickPlanner（138 项）
...
- 不在本文虚构远端地址或 CI；所有变更以本地 `swift build -c release` 与 `make test`（360 项）验证为准。
