...
- **自适应跟随**：macOS 14+ 跟随底座窗口所在屏幕的原生显示节拍；macOS 13 按当前屏幕刷新能力选择 repeating Timer，并上限为 120 Hz。密集回调只保留最新的一个待执行 tick；静止判定在 60/120 Hz 和不规则节拍下保持相同的实际时长语义，随后降为 0.1 秒一次的静止探测。宠物隐藏或 Codex 退出时底座与详情同步隐藏，重现后重新捕获。
