...
- `make app` 自动优先本机稳定自签证书（默认 `PetDock Local Development`，可用 `STABLE_SIGN_IDENTITY` 覆盖），未命中或签名失败时回落 ad-hoc 并明确提示；该命令仍只在**最终 QA / 发布阶段**执行，减少 TCC 重授权次数。
...
- 分发包优先为稳定自签证书签名（无 Developer ID、未公证），无证书环境回落 ad-hoc；README 仍须含 **Preview + Open Anyway** 安装说明。
