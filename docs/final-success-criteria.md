...
`make test` = test-ui + test-data + test-shell，**306 项全部通过，0 failed**。

| 套件 | 项数 | 覆盖 |
| --- | --- | --- |
| test-ui | 113 | selectPet 识别（Mascot 优先 / 合理回退排除辅助窗）+ unionCandidates 单次枚举/PID 缓存 + Geometry 坐标（多屏 / 负坐标）+ Follower 状态机（亚像素容差）+ Dock 几何/reset + **BubbleVisibility 32（分类滞回 + 调度 + 异步 generation single-flight + 空闲态空 probe 早退）** + **障碍避让 17（链式/排除/越界/恢复）** + **水平 clamp 6** + **DetailPanel clamp/fullScreenAuxiliary 5** + **PetLogger 异步门控 6（no-op / 异步写 / 落盘 + 默认构造跟随 --verbose / no-op 不回归）** |
