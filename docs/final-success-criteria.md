...
`make test` = test-ui + test-data + test-shell，**304 项全部通过，0 failed**。

| 套件 | 项数 | 覆盖 |
| --- | --- | --- |
| test-ui | 111 | selectPet 识别（Mascot 优先 / 合理回退排除辅助窗）+ unionCandidates 单次枚举/PID 缓存 + Geometry 坐标（多屏 / 负坐标）+ Follower 状态机（亚像素容差）+ Dock 几何/reset + **BubbleVisibility 32（分类滞回 + 调度 + 异步 generation single-flight + 空闲态空 probe 早退）** + **障碍避让 17（链式/排除/越界/恢复）** + **水平 clamp 6** + **DetailPanel clamp/fullScreenAuxiliary 5** + **PetLogger 异步门控 4** |
| test-data | 100 | Token 聚合Σ / 脱敏 / 分项、增量缓存、缓存淘汰、parseLine 鲁棒、WeekLeft 解析 / 窗口 / 重置 / cancel、退避表、pause 语义、service 端到端、LiveDockProvider 映射、并发安全、codex 路径解析 + 子进程 PATH + resetsAt 格式化 + **LineReader 异步读 5（stop 不挂起/EOF/不完整行缓冲）** |
| test-shell | 93 | Theme 内置 / 外部安全解析（颜色 / 字体 / 徽标 / 危险关键字）、Settings 持久化、ThemeStore fixture 热加载、AutoStart 状态映射、**StatusBar TCC 权限降级提示** |
