...
任意生产 sink 由编译层合同封死，而非文本扫描：collector 的 designated initializer 为 `private`（同文件工厂专属）；唯一生产构造入口是同文件 `production(candidateSHA:flushNow:)` 工厂，签名不含输出地址参数，落盘位置固定为 PetDock 私有 Diagnostics 证据文件；测试自定义临时 sink 只能经 `#if PETDOCK_TESTING` 包裹的同文件 `forTesting` 工厂进入，该 flag 仅出现在 test-ui 的独立 `swiftc` 编译命令中，`Package.swift` 不定义它，release 构建在词法阶段即排除测试工厂。外部直接构造、显式 `.init`、注释分隔构造、生产调用传入输出地址参数或 release 引用测试工厂都以 release 编译失败作为主证据；`make test-privacy` 中的 W0–W7 文本守卫只承担 accidental-drift 与 flag/布线计数（单一 token、declaration shape、精确计数 allowlist，其中 W7 对定义文件 test flag 区域外钉死完整 declaration 清单并禁止 extension token），不得被报告为访问控制证明。
